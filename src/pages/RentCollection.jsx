...
import UtilityChargeCard from "../components/utilityChargeCard.jsx";
import UtilityChargeModal from "../components/modals/utilityChargeModal.jsx";
import { rentCollectionAPI } from "../utils/rentCollectionApi.jsx";
...
const RentCollection = () => {
  ...
  const [selectedUtilityCharge, setSelectedUtilityCharge] = useState(null);
  const [showUtilityModal, setShowUtilityModal] = useState(false);
  const [showGenerateUtilitiesModal, setShowGenerateUtilitiesModal] =
    useState(false);
  ...
  const [utilityPagination, setUtilityPagination] = useState({
    currentPage: 1,
    totalPages: 1,
    totalCount: 0,
    limit: 10,
  ...
  const handleBillUtilitiesToRent = async (month, year) => {
    try {
      setProcessing(true);
      const result = await rentCollectionAPI.billUtilitiesToRent(month, year);

      if (result.status === 200) {
        alert(
          `${result.message}\n${result.data.billed_count} utilities billed successfully!`
        );
        loadUtilityCharges();
        loadUtilitySummary();
        if (activeTab === "payments") {
      ...
      alert("Error billing utilities to rent: " + error.message);
    } finally {
      setProcessing(false);
    }
  };
  ...
  const handleProcessPayment = async (paymentId, paymentData) => {
    try {
      setProcessing(true);
      const result = await rentCollectionAPI.processPayment(
        paymentId,
        paymentData
      );
      if (result.success) {
        setPayments((prev) =>
          prev.map((p) => (p.id === paymentId ? { ...p, ...result.data } : p))
        );
        loadSummary();
  ...
  const handleCreatePayment = async (paymentData) => {
    ...
    try {
      ...
      if (createResult.success) {
        const processResult = await rentCollectionAPI.processPayment(
  ...
  const handleVerifyPayment = async (submissionId, verificationData) => {
    try {
      setProcessing(true);
      const result = await rentCollectionAPI.verifyPaymentSubmission(
        submissionId,
        verificationData
      );
  ...
  const handleRejectPayment = async (submissionId, rejectionData) => {
    try {
      setProcessing(true);
      const result = await rentCollectionAPI.rejectPaymentSubmission(
        submissionId,
        rejectionData
      );
  ...
  const handleBulkVerification = async (submissionIds, bulkData) => {
    try {
      ...
      if (bulkData.action === "verify") {
        const result = await rentCollectionAPI.bulkVerifySubmissions(
          submissionIds,
          {
        ...
        if (result.success || result.status === 200) {
          setPendingSubmissions((prev) =>
            prev.filter((sub) => !submissionIds.includes(sub.id))
          );
          alert(
            `${submissionIds.length} payment${
        ...
        if (successful > 0) {
          ...
          if (failed === 0) {
            ...
            alert(
              `${successful} payments rejected successfully, ${failed} failed.`
            );
          }
        } else {
  ...
  const handleViewSubmissionDetails = async (submission) => {
    try {
      const result = await rentCollectionAPI.getSubmissionDetails(
        submission.id
      );
  ...
  const handleSendReminders = async () => {
    ...
    try {
      const result = await rentCollectionAPI.sendReminders(
        overduePayments,
        "overdue"
      );
      if (result.success) {
  ...
  const loadUtilitySummary = async () => {
    try {
      const result = await rentCollectionAPI.getUtilitySummary({
        month: utilityFilters.month,
      ...
      console.error("Error loading utility summary:", error);
    }
  };

  const handleUtilityFilterChange = (key, value) => {
  ...
  const handleCreateUtilityCharge = async (data) => {
    try {
      setProcessing(true);
      const result = await rentCollectionAPI.createUtilityCharge(data);
      if (result.status === 201) {
        alert("Utility charge created successfully!");
        loadUtilityCharges();
        loadUtilitySummary();
      ...
      alert("Error creating utility charge: " + error.message);
    } finally {
      setProcessing(false);
    }
  };

  const handleUpdateUtilityCharge = async (id, data) => {
    try {
      setProcessing(true);
      const result = await rentCollectionAPI.updateUtilityCharge(id, data);
      if (result.status === 200) {
        alert("Utility charge updated successfully!");
        loadUtilityCharges();
        loadUtilitySummary();
        setShowUtilityModal(false);
        setSelectedUtilityCharge(null);
      } else {
        alert("Error: " + result.message);
      }
    } catch (error) {
      alert("Error updating utility charge: " + error.message);
    } finally {
      setProcessing(false);
    }
  };

  const handleGenerateUtilityCharges = async (month, year, defaultCharges) => {
    try {
      setProcessing(true);
      const result = await rentCollectionAPI.generateUtilityCharges(
        month,
        year,
        defaultCharges
      );
      if (result.status === 201) {
        ...
        loadUtilityCharges();
        loadUtilitySummary();
        setShowGenerateUtilitiesModal(false);
      } else {
        alert("Error: " + result.message);
      }
    } catch (error) {
      alert("Error generating utility charges: " + error.message);
    } finally {
      setProcessing(false);
    }
  };
    ...
    <Navbar module={activeModule}>
      <div className="space-y-6">
              ...
              <>
                ...
                {verificationPagination.totalPages > 1 && (
                  <div className="flex justify-center space-x-2">
                    ...
                    {Array.from(
                      { length: verificationPagination.totalPages },
                      (_, i) => i + 1
                    ).map((page) => (
                      <button
                        key={page}
                        onClick={() =>
                          handleVerificationFilterChange("page", page)
                        }
                        className={`px-3 py-1 border rounded ${
                          page === verificationPagination.currentPage
                            ? "bg-blue-500 text-white"
                            : ""
                        }`}
                      >
                      ...
                      <div className="text-right">
                        ...
                        >
                          {submission.verification_status
                            .charAt(0)
                            .toUpperCase() +
                            submission.verification_status.slice(1)}
                        </span>
              ...
              <div className="bg-white p-4 rounded-lg shadow">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold">
                      {formatCurrency(utilitySummary.total_amount || 0)}
                    </p>
                  </div>
                  <DollarSign className="w-8 h-8 text-blue-500" />
                </div>
              ...
              <div className="bg-white p-4 rounded-lg shadow">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-gray-600">Collected</p>
                    <p className="text-2xl font-bold text-green-600">
                      {formatCurrency(utilitySummary.total_collected || 0)}
                    </p>
                  </div>
                  <CheckCircle className="w-8 h-8 text-green-500" />
                </div>
              ...
              <div className="bg-white p-4 rounded-lg shadow">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-gray-600">Pending</p>
                    <p className="text-2xl font-bold text-yellow-600">
                      {formatCurrency(utilitySummary.total_pending || 0)}
                    </p>
                  </div>
                  <Clock className="w-8 h-8 text-yellow-500" />
                </div>
              ...
              <div className="bg-white p-4 rounded-lg shadow">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-gray-600">Overdue</p>
                    <p className="text-2xl font-bold text-red-600">
                      {formatCurrency(utilitySummary.total_overdue || 0)}
                    </p>
                  </div>
                  <AlertTriangle className="w-8 h-8 text-red-500" />
                </div>
              ...
              <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center space-y-4 lg:space-y-0">
                <div className="flex flex-wrap items-center space-x-4">
                  <div>
                    <select
                      className="border rounded px-3 py-2"
                      value={utilityFilters.status}
                      onChange={(e) =>
                        handleUtilityFilterChange("status", e.target.value)
                      }
                    >
                      <option value="all">All Status</option>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0">
              <div className="flex flex-wrap space-x-2 space-y-2 lg:space-y-0">
                <button
                  ...
                  className="bg-blue-500 text-white px-4 py-2 rounded flex items-center hover:bg-blue-600"
                >
                  <Plus className="mr-2 w-4 h-4" /> Add Utility Charge
                </button>
                <button
                  onClick={() => setShowGenerateUtilitiesModal(true)}
                  className="bg-green-500 text-white px-4 py-2 rounded flex items-center hover:bg-green-600"
                >
                  <Calendar className="mr-2 w-4 h-4" /> Generate Monthly
                </button>
                <button
                  onClick={() =>
                    handleBillUtilitiesToRent(
                      utilityFilters.month,
                      utilityFilters.year
                    )
                  }
              ...
              <div className="text-sm text-gray-600">
                Total Records: {utilityPagination.totalCount || 0}
              </div>
            </div>
            ...
            {utilityLoading ? (
              ...
              <div className="text-center py-8 bg-white rounded-lg shadow">
                ...
                <p className="text-gray-600 mb-4">
                  Start by adding utility charges or generating them for the
                  month.
                </p>
                <button
                  onClick={() => {
                  ...
                  className="bg-blue-500 text-white px-6 py-2 rounded hover:bg-blue-600 inline-flex items-center"
                >
                  <Plus className="mr-2 w-4 h-4" /> Add First Utility Charge
                </button>
              </div>
            ) : (
              <>
                <div className="grid md:grid-cols-2 gap-6">
                  {utilityCharges.map((charge) => (
                    <UtilityChargeCard
                      ...
                      charge={charge}
                      onEdit={(charge) => {
                        setSelectedUtilityCharge(charge);
                        setShowUtilityModal(true);
                      }}
                    />
                  ))}
                  ...
                  <div className="flex justify-center space-x-2">
                    <button
                      onClick={() =>
                        handleUtilityFilterChange(
                          "page",
                          utilityPagination.currentPage - 1
                        )
                      }
        ...
        <UtilityChargeModal
          ...
          charge={selectedUtilityCharge}
          activeLeases={activeLeases}
          onSubmit={
            selectedUtilityCharge
              ? handleUpdateUtilityCharge
              : handleCreateUtilityCharge
          }
          processing={processing}
        />

        <GeneratePaymentsModal
          isOpen={showGenerateUtilitiesModal}
          onClose={() => setShowGenerateUtilitiesModal(false)}
          onSubmit={handleGenerateUtilityCharges}
          processing={processing}
          title="Generate Utility Charges"
          description="Generate utility charges for all active leases"
        />
      </div>
    </Navbar>
  );
