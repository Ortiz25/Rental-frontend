...
const PropertyManagement = () => {  
  ...
  const [filters, setFilters] = useState({
    ...
    const totalPortfolioStats = {
      ...
      totalVacantUnits: properties.reduce((sum, prop) => sum + prop.vacantUnits, 0),
      occupancyRate: (properties.reduce((sum, prop) => sum + prop.occupiedUnits, 0) / 
                      properties.reduce((sum, prop) => sum + prop.totalUnits, 0) * 100).toFixed(2)
    };

  useEffect(() => {
    const results = properties.filter(property => 
      property.address.toLowerCase().includes(searchTerm.toLowerCase()) &&
      property.monthlyRent >= filters.minRent &&
      property.monthlyRent <= filters.maxRent &&
      (filters.bedrooms === '' || property.bedrooms === parseInt(filters.bedrooms)) &&
      (filters.occupancyState === '' || property.occupancyState === filters.occupancyState) &&
      property.vacantUnits >= filters.minVacancies
    );
  ...
  <div className="text-center">
    ...
    <p className="text-xl font-bold">{totalPortfolioStats.totalProperties}</p>
  </div>
  <div className="text-center">
    <p className="text-gray-600">Total Units</p>
    <p className="text-xl font-bold">{totalPortfolioStats.totalUnits}</p>
  </div>
  <div className="text-center">
    <p className="text-gray-600">Occupied Units</p>
    <p className="text-xl font-bold text-green-600">{totalPortfolioStats.totalOccupiedUnits}</p>
  </div>
  <div className="text-center">
    <p className="text-gray-600">Vacant Units</p>
    <p className="text-xl font-bold text-red-600">{totalPortfolioStats.totalVacantUnits}</p>
  </div>
  <div className="text-center">
    <p className="text-gray-600">Overall Occupancy</p>
    <p className="text-xl font-bold">{totalPortfolioStats.occupancyRate}%</p>
  </div>
</div>
  ...
  <button className="bg-blue-500 text-white px-4 py-2 rounded flex items-center justify-center sm:w-auto w-full">
  ...
  <select 
    ...
    onChange={(e) => setFilters({ ...filters, occupancyState: e.target.value })}
  >
  ...
  <input 
    ...
    onChange={(e) => setFilters({ ...filters, minVacancies: Number(e.target.value) })}
  />
