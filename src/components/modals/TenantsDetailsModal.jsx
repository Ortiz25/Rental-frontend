  ...
  const DocumentsTab = ({ tenant }) => {
    ...
    const fetchDocuments = async () => {
      ...
      try {
        ...
        const response = await fetch(`/backend/tenants/${tenant.id}/documents`, {
          headers: {
    ...
    const handleDownload = async (documentId, documentName) => {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`/backend/tenants/documents/${documentId}/download`, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
    ...
    const handleView = async (documentId, documentName, mimeType) => {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`/backend/tenants/documents/${documentId}/view`, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
    ...
    const handleUpload = async () => {
      ...
      try {
        ...
        const response = await fetch('/backend/tenants/tenant/upload', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`
          },
